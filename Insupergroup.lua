...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "demoteleader" then
		local user_id = result.from.peer_id
		local channel_id = "channel#id"..result.to.peer_id
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'demoteleader' then
			if not is_support(msg.from.id) and not is_owner(msg) then
				return
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demoteleader',
					msg = msg
				}
				demoteadmin = get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'demoteleader' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'demoteleader'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'demoteleader' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demoteleader'
				}
